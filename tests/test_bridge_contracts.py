...
class BridgeContractTests(unittest.TestCase):
    ...
    def test_mqtt_discovery_contract(self):
        ...
        self.assertIn(
            "homeassistant/binary_sensor/weber_connect_testserial_connectivity/config",
            config_topics,
        )
        self.assertIn(
            "homeassistant/sensor/weber_connect_testserial_last_publish/config",
            config_topics,
        )
        self.assertNotIn(
            "homeassistant/sensor/weber_connect_testserial_probe_2_battery/config",
            config_topics,
        )
        self.assertTrue(all(row["retain"] for row in plan[:-1]))
        self.assertTrue(all(row["payload"] for row in plan[:-1]))
    ...
    def test_persistent_runtime_discovery_includes_availability(self):
        ...
        self.assertEqual(
            ...
            "weber_connect/weber_connect_testserial/probe_1/availability",
        )
