...
import weber_cloud_socket as socket  # noqa: E402
...
def program_payload() -> bytes:
    ...
    return header + josl("Weeknight Steak") + bytes([2]) + struct.pack("<H", 1) + step + struct.pack("<H", 1) + prompt
...
class EnvelopeTests(unittest.TestCase):
    def test_targeted_envelope_round_trip(self) -> None:
        encoded = socket.encode_routed_message(
            COMPANION_ID, APPLIANCE_ID, 123, 0x0B, b"\x01\x03"
        )
    ...
    def test_invalid_ids_and_transport_length_are_rejected(self) -> None:
        ...
        valid = bytearray(
            socket.encode_routed_message(COMPANION_ID, APPLIANCE_ID, 1, 5)
        )
...
class ProgramTests(unittest.TestCase):
    ...
    def test_v10_empty_program_and_inactive_mapping(self) -> None:
        ...
        self.assertFalse(
            socket.active_cook_from_program({"probes": []}, program)["active"]
        )
    ...
    def test_invalid_text_and_excessive_counts_are_rejected(self) -> None:
        ...
        with self.assertRaises(socket.WeberCloudSocketError):
            socket.decode_program_details(
                header
                + josl("Too many prompts")
                + b"\x00"
                + struct.pack("<HH", 0, 1025),
                11,
            )
...
class ClientTests(unittest.TestCase):
    ...
    def test_receive_skips_text_rejects_errors_and_closes_on_failure(self) -> None:
        accepted = socket.encode_routed_message(
            APPLIANCE_ID, COMPANION_ID, 1, 0x80, b""
        )
